...
public class MeasurementClient {
    ...
    public static void main(String[] args) {
        ...
        Configuration ramCloudGraphConfig = new BaseConfiguration();
        Configuration testConfig = new BaseConfiguration();
        ...
        options.addOption("S", "servers", true, "Total master servers.");
        options.addOption("D", "duration", true, "Duration of each test.");
        ...
        // RAMCloud configuration parameters.
        if(!cmd.hasOption("C")) {
            logger.log(Level.SEVERE, "Missing required argument: coordinator");
            return;
        }
        
        coordinatorLocator = cmd.getOptionValue("C");
        
        if(!cmd.hasOption("S")) {
            logger.log(Level.SEVERE, "Missing required argument: servers");
            return;
        }
        
        totalMasterServers = Integer.decode(cmd.getOptionValue("S"));
        
        ramCloudGraphConfig.setProperty(RAMCloudGraph.CONFIG_COORD_LOC, coordinatorLocator);
        ramCloudGraphConfig.setProperty(RAMCloudGraph.CONFIG_NUM_MASTER_SERVERS, totalMasterServers);
        
        // Test configuration parameters.
        if(!cmd.hasOption("D")) {
            logger.log(Level.SEVERE, "Missing required argument: duration");
            return;
        }
        
        testDuration = Long.decode(cmd.getOptionValue("D"));
